...
class AuthController {
    ...
    static async login(req, res) {
        try {
            ...
            if (req.body.approved && req.body.userId) {
                ...
                await User.attendance(user.user_id, currentSession);

                // Get updated attendance count and emit to all clients
                const attendance = await User.senatepresent(currentSession);
                const attendanceCount = attendance[0]['COUNT(*)'];
                req.app.get('io').emit('attendanceUpdate', { count: attendanceCount });
            ...
            if (sessionStatus !== 'ongoing') {
                return res.render('auth/login', {
                    error: 'No active session is running. Please wait for an Secretary to start a session.',
                    query: req.query
                });
            }
    ...
    static async showDashboard(req, res) {
        try {
            ...
            let pendingUsers = [];
            let attendanceCount = 0; // Initialize outside if block
            ...
            res.render('dashboard', { 
                user, 
                pendingUsers,
                attendanceCount // Now defined for both admin and non-admin users
            });
        } catch (error) {
