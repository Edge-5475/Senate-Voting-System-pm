    ...
    static async createSession(req, res) {
        try {
            ...
            if (!['Secretary', 'VicePresident'].includes(user.role_name)) {
                return res.status(403).send('Must be Secretary or VicePresident to create session');
            }
            ...
            if (error.message === 'Session already exists') {
                res.status(400).send('A session is already in progress');
            } else {
